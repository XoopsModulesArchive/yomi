...
if (empty($EST['shorturl'])) {
    define('YOMI_HOME_URL', '<a href="' . $EST['home'] . '">トップ</a>');
} else {
    define('YOMI_HOME_URL', '<a href="' . XOOPS_URL . '/' . $EST['shorturl'] . '/">トップ</a>');
}
...
function PR_other_kt($path = '')
{
    ...
    if (count($gane_other) >= 0) {
        ?><a name=other></a>【その他のカテゴリ】<font id=small>[<a href="<?= $path ?>#top">↑ページTOPへ</a>]</font>
        <center>
            <table style="width:90%" cellpadding=3 id=mid>
                <?php
...
function put_icon()
{
    ...
    if (0 == $Slog[13] && $times - $Slog[11] < 86400 * $EST['new_time']) {
        ##新着マーク
        ?>
...
function mokuji($arg)
{
    ...
    if ($arg[1] > $arg[2]) { #目次作成
        ...
        if (1 != $max_page_f - ($pre_page_f + $pre_page_a - 1) or $max_page_a) {
        ...
        if ($arg[0] < $max_page) {
            if ($EST['html'] && $ganes[$_GET['kt']]) {
                $md_url = (string)$_GET[kt] . 'p';
            }
...
function cp_cr()
{
    ...
    <div style="text-align:right;font-size:11px"><a href="http://hypweb.net/" target="_blank">Yomi-Search [ XOOPS ] Ver. <?= $x_ver ?></a><br>Based on - <a href="http://yomi.pekori.to/" target="_blank">Yomi-Search Ver<?= $ver ?></a> - <a href="http://sql.s28.xrea.com:8080/" target="_blank">Powered
            by PHP</a></div>
    <?php
    //For XOOPS
    echo '</td></tr></table>';
...
function full_kt($arg)
{
    ...
    $Rkt_name = mb_substr($Rkt_name, 0, -1);
...
function pass_check()
{
    ...
    if ($EST[login_ip]) {
        ...
        foreach ($ip_list as $ip) {
            if (mb_strstr($_SERVER['REMOTE_ADDR'], $ip)) {
                $fl = 1;

                break;
            } elseif (mb_strstr($_SERVER['REMOTE_HOST'], $ip)) {
                $fl = 1;
    ...
    if (1 != $is_admin) {
        ...
        if ($EST[pass] != $cr_pass || (!$_POST['pass'])) { #パスワードが不一致
            ...
            $fp = fopen("$EST[log_path]pass_check.cgi", 'ab');

            fwrite($fp, "$_SERVER[REMOTE_ADDR]<>$_SERVER[REMOTE_HOST]<>$date<>$_POST[pass]<>\n");
    ...
    <table width="100%" cellpadding=8 cellspacing=0 border=0>
        ...
        <tr>
            ...
            <td align=right><font id=small>
                    ...
                    <?php
                    if (!isset($gane_UR[$_GET['kt']]) && 'kt' == $_GET['mode']) {
                        ?>

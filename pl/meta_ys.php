    ...
    */
    ...
    if (is_array($data)) {
        ...
        foreach ($data as $line) {
            if (0 === mb_strpos($line, '//') || 0 === mb_strpos($line, '-')) {
                continue;
            }
            ...
            if (!empty($_GET['word'])) {
                ...
                if (!empty($lines[6])) {
                    if ('sjis' == $lines[6]) {
                        $word = mb_convert_encoding($_GET['word'], 'SJIS', 'EUC-JP');
                    }

                    if ('utf8' == $lines[6]) {
                        $word = mb_convert_encoding($_GET['word'], 'UTF-8', 'EUC-JP');
                    }
                }

                if ('and' == $_GET['method']) {
                    if (!empty($lines[7])) {
                        $word = str_replace(' ', $lines[7], $word);
                    }
                } else {
                    if (!empty($lines[8])) {
                        $word = str_replace(' ', $lines[8], $word);
                    }
                }

                $word = $lines[4] . '=' . urlencode($word);

                if (!empty($_GET['hyouji']) && !empty($lines[9])) {
                    $option[$lines[0]] .= '&' . $lines[9] . '=' . htmlspecialchars($_GET['hyouji'], ENT_QUOTES | ENT_HTML5);
                }
            }
    ...
    if ('select' == $T_mode) {
        ...
        if (!$_GET['word']) {
            ...
            if ('com' == $_GET['engine'] || 'cojp' == $_GET['engine']) { #?&をカット
        ...
        foreach ($name as $tmp) {
            if ('on' == $_GET[$tmp] || 'meta_page' == $T_mode) {
                if (!$_GET['word']) {
                    $location_list[] = "{$engine_name[$tmp]}<>http://{$engine_top[$tmp]}<>\n";
                } else {
                    $url = str_replace('&', '&amp;', "http://{$engine[$tmp]}&{$keyword[$tmp]}{$option[$tmp]}");

                    $location_list[] = "{$engine_name[$tmp]}<>{$url}<>\n";
                }
            }
        }
    }

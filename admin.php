...
require_once './include/hyp_tickets.php';
...
//if (!xoopsSecurity::checkReferer()) redirect_header(XOOPS_URL."/",1,"Access Denied.");

if (('dl_check_dl' == $_POST['mode']) || ('log_conv_act' == $_POST['mode'])) {
} else {
    xoops_cp_header();
...
elseif ('temp_to_regist' == $_POST['mode']) {
    #(5)登録待ち表示画面(&temp_to_regist)
    ...
    exit;
} elseif ('temp_to_regist_act' == $_POST['mode']) {
    #(5.1)登録待ちの処理決定実行(&temp_to_regist_act)
    ...
    while ($tmp = fgets($fp, 4096)) {
        ...
        if ('reg' == $_POST["R$Tlog[0]"]) { #登録
            ...
            for ($i = 1; $i <= 2; $i++) { #←マーク数を増やす場合には修正
                if (!$_POST["Fmark${i}_$temp_id"]) {
                    $_POST["Fmark${i}_$temp_id"] = 0;
                }

                $Slog[3] .= $_POST["Fmark${i}_$temp_id"] . '_';
            }

            $Slog[3] = mb_substr($Slog[3], 0, -1);
            ...
            $result = $xoopsDB->queryF($query) || die("Query failed admin146 $query");
    ...
    $fp = fopen("$EST[log_path]$EST[temp_logfile]", 'wb');
    ...
    $fp = fopen("$EST[log_path]keyrank_ys.php", 'wb');

    if ($_POST[open]) {
        foreach ($_POST[open] as $key) {
            if ('on' != $_POST["del_oo_$key"]) {
                $open_key[$key] = $key;
            } #open
        }
    }
    ...
    foreach ($open_key as $key => $val) {
        if (!$check[$key]) {
            if (!$_POST["hm_$key"] && '1' != $val) {
                fwrite($fp, "'$key'=>'1',\n");
            } elseif (!$_POST["hm_$key"]) {
                fwrite($fp, "'$key'=>'$val',\n");
            } else {
                fwrite($fp, "'key'=>'" . $_POST["hm_$key"] . "',\n");
            }
    ...
    exit;
} elseif ('key_cfg_del_word_act' == $_POST['mode']) {
    #(6.2)キーワードランキングの集計対象外のキーワードを一括登録実行(&key_cfg_del_word_act)
    ...
    exit;
} elseif ('log_conv_act' == $_POST['mode']) {
    #(7.1)各種ログ変換実行(&log_conv_act)
    ...
    if ('on' != $_POST[check]) {
        mes_frame('確認チェックがされていません。<br>チェックしてから実行してください。', 'チェックエラー');
    }

    if (!is_file($_POST[bf_file])) {
        mes_frame("エラー：$bf_file が見つかりません", 'ファイルが見つかりません', 'java');
    } else {
        if ('v4todb' == $_POST[log_mode]) {
            // 実行時間を制限しない
            ...
            while ($line = fgets($fp, 4096)) {
                ...
                $Slog[13] = mb_substr($Slog[11], -1);

                $Slog[11] = mb_substr($Slog[11], 0, -2);
...
elseif ('log_conv_kt_sort' == $_POST['mode']) {
    #(7.3)カテゴリ・ソート変換実行(&log_conv_kt_sort)
    ...
    while (list($key,) = each($ch_cnt)) {
        [$top_kt] = explode('_', $key);

        unset($furi);

        if ($ch_cnt[$key] > 1 && 'on' == $_POST[$top_kt]) {
            $kt = explode(',', $ch_line[$key]);

            foreach ($kt as $tmp) {
                if ($tmp) {
                    $furi[$tmp] = $EST_furi[$tmp];
                }
            }
    ...
    $result = $xoopsDB->query($query) || die("Query failed admin440 $query");
    ...
    while (false !== ($Slog = $GLOBALS['xoopsDB']->fetchRow($result))) {
        ...
        if ($kt_fl) {
            ...
            $result2 = $xoopsDB->queryF($query) || die("Query failed admin449 $query");
        }
    }

    mes('カテゴリ・ソート変換が完了しました', 'カテゴリ・ソート変換完了', 'kanri');
} elseif ('log_kt_change' == $_POST['mode']) {
    #(8)ログデータの交換・移動・削除(&log_kt_change)
    ...
    exit;
} elseif ('log_kt_change_act' == $_POST['mode']) {
    #(8.1)ログデータの交換・移動・削除実行(&log_kt_change_act)
    ...
    if ('change' == $_POST[log_mode]) { #change
        #ログデータの交換
        ...
        $kousin_kt = ["&$change_kt1&" => "&$change_kt2&", "&$change_kt2&" => "&$change_kt1&"];

        $query = "SELECT id,category FROM $EST[sqltb]log WHERE (category LIKE '%&$change_kt1&%') OR (category LIKE '%&$change_kt2&%')";

        $result = $xoopsDB->query($query) || die("Query failed admin216 $query");

        while (false !== ($line = $GLOBALS['xoopsDB']->fetchBoth($result, MYSQL_ASSOC))) {
            $line[category] = strtr($line[category], $kousin_kt);

            $query = "UPDATE $EST[sqltb]log SET category='$line[category]' WHERE id='$line[id]'";

            $result2 = $xoopsDB->queryF($query) || die("Query failed admin220 $query");
        }
    } elseif ('move' == $_POST[log_mode]) { #move
        #ログデータの移動
        ...
        $result = $xoopsDB->query($query) || die("Query failed admin232 $query");
        ...
        while (false !== ($line = $GLOBALS['xoopsDB']->fetchBoth($result, MYSQL_ASSOC))) {
            ...
            $result2 = $xoopsDB->queryF($query) || die("Query failed admin236 $query");
        }
    } else { #del
        #ログデータの削除
        ...
        while (false !== ($line = $GLOBALS['xoopsDB']->fetchBoth($result, MYSQL_ASSOC))) {
            ...
            $result2 = $xoopsDB->queryF($query) || die("Query failed admin256 $query");
        }
    }

    mes($PR_mes, 'ログデータの交換・移動・削除完了', 'kanri');
} elseif ('log_repair' == $_POST['mode']) {
    #(9)ログ(登録データ)の修復(&log_repair)

    require "$EST[temp_path]admin/log_repair.html";

    exit;
} elseif ('log_repair_act' == $_POST['mode']) {
    #(9.1)ログ(登録データ)の修復実行(&log_repair_act)
    ...
    if ('dump' == $_POST[act]) {
        ...
        $result = $xoopsDB->query($query) || die("Query failed admin378 $query");
        ...
        $result = $xoopsDB->queryF($query) || die("Query failed admin392 $query");
        ...
        $result = $xoopsDB->queryF($query) || die("Query failed admin411 $query");
    ...
    exit;
} elseif ('log_mente' == $_POST['mode']) {
    log_mente;
} elseif ('log_mente_act' == $_POST['mode']) {
    log_mente_act;
} elseif ('config' == $_POST['mode']) {
    #(11)環境設定 (&config)

    require "$EST[temp_path]admin/config.html";

    exit;
} elseif ('config_kt' == $_POST['mode']) {
    #(12)カテゴリ設定 (&config_kt)

    require "$EST[temp_path]admin/config_kt.html";

    exit;
} elseif ('rank_cfg' == $_POST['mode']) {
    #(13)人気ランキングの設定(&rank_cfg)
...
elseif ('dl_check' == $_POST['mode']) {
    ...
    $fp = fopen("$EST[log_path]no_link_temp.cgi", 'rb');
    ...
    while ($tmp = fgets($fp, 4096)) {
        ...
        if (!$fl["$data[0]_$data[1]"]) {
            ...
            $result = $xoopsDB->query($query) || die("Query failed admin356 $query");

            $Slog = $GLOBALS['xoopsDB']->fetchArray($result);

            for ($i = 1; $i <= 5; $i++) {
                if ($count["${i]_$Slog[id]"}) {
                    $url[$Slog[id]] = $Slog[url];
    ...
    if (isset($url)) {
        ...
        foreach ($url as $id => $val) {
            ...
            for ($i = 1; $i <= 5; $i++) {
                ...
                fwrite($fp, $count["${i}_$id"] . ',');
            }

            fwrite($fp, " <> $PR_ip[$id]<>$val <> $com[$id]<>\n");
        }

        fclose($fp);
    }

    $fp = fopen("$EST[log_path]no_link_temp . cgi", 'wb');
    ...
    $result = $xoopsDB->query($query) || die("Query failed admin682 $query");
    ...
    exit;
} elseif ('dl_check_act' == $_POST['mode']) {
    #(14.2)デッドリンクチェック実行画面(&dl_check_act)
    ...
    while ($tmp = fgets($fp, 4096)) {
        ...
        if (mb_strstr($data[13], 'Not Found') || mb_strstr($data[13], 'Forbidden')) {
            $url[$data[0]] = $data[1];
...
elseif ('ver_info' == $_POST['mode']) {
    ...
    if ($_POST[new_pass]) {
        $bf_pass = $_POST[pass] = $_POST[new_pass];
    }
    ...
    exit;
} elseif ('cfg_make_PR_menu' == $_POST['mode']) {
    #(cfg2)環境設定(&search_form/&menu_bar)を更新(&cfg_make_PR_menu)
    ...
    if (!trim($_POST['search_form'])) {
        $_POST['search_form'] = get_search_form();
    }
    ...
    foreach ($file as $tmp) {
        ...
        if (1 == $fl) {
            array_push($file_data, $_POST[search_form]);

            $fl = 0;
        } elseif (2 == $fl) {
            array_push($file_data, $_POST[menu_bar]);

            $fl = 0;
        } elseif (3 == $fl) {
            array_push($file_data, $_POST[head_sp]);

            $fl = 0;
        } elseif (4 == $fl) {
            array_push($file_data, $_POST[foot_sp]);

            $fl = 0;
        }

        if ('} #end of &search_form' == $tmp) {
            $file_data[] = '<?php';

            $p_fl = 1;
        } elseif ('} #end of &menu_bar' == $tmp) {
            $file_data[] = '<?php';

            $p_fl = 1;
        } elseif ('} #end of &head_sp' == $tmp) {
            $file_data[] = '<?php';

            $p_fl = 1;
        } elseif ('} #end of &foot_sp' == $tmp) {
            $file_data[] = '<?php';
        ...
        if ('function search_form(){' == $tmp) { #search
            $file_data[] = 'global $EST;';
            ...
            $p_fl = 0;
        } elseif ('function menu_bar(){' == $tmp) { #menu
            $file_data[] = 'global $EST;';
            ...
            $p_fl = 0;
        } elseif ('function head_sp(){' == $tmp) { #head
            $file_data[] = '?>';

            $fl = 3;

            $p_fl = 0;
        } elseif ('function foot_sp(){' == $tmp) { #foot
            $file_data[] = '?>';
    ...
    exit;
} elseif ('cfg_make_reg' == $_POST['mode']) {
    #(cfg3)環境設定(登録処理関係)を更新 (&cfg_make_reg)
    ...
    exit;
} elseif ('cfg_make_kt_ex' == $_POST['mode']) {
    #(cfg4)カテゴリ説明文を更新 (&cfg_make_kt_ex)
    ...
    foreach ($KTEX as $key => $val) { #環境設定(%KTEX)を更新
        if ($_POST["d_$key"]) {
            unset($KTEX[$key]);
        } elseif (isset($_POST["ex_$key"])) {
            $KTEX[$key] = $_POST["ex_$key"];
        }
    }
    ...
    if ($_POST[ktex]) {
        $_POST[ktex] = str_replace(['&gt;', '&lt;'], ['>', '<'], $_POST[ktex]);
    ...
    if ('mente' == $_POST['mente_mode']) {
        ...
        foreach ($ganes as $key => $val) { #カテゴリ設定(%ganes)を更新
            if ($_POST["d_$key"]) { #削除
                ...
                $_key = array_search($key, $gane_other, true);

                if (false !== $_key) {
                    array_splice($gane_other, $_key);
                }
            } elseif (isset($_POST["kt_$key"])) { #カテゴリがあれば
                ...
                elseif (1 !== $gane_UR[$key]) {
                    unset($gane_UR[$key]);
                }

                if ($_POST["ref_$key"]) {
                    $gane_ref[$key] = $_POST["ref_$key"];
                } elseif ('' === $gane_ref[$key]) {
                    unset($gane_ref[$key]);
                }
            }
    ...
    if ('new' == $_POST['mente_mode']) {
        #新規追加分を定義

        if ($_POST['kt_new']) {
            $_POST['kt_new'] = trim(str_replace(['&gt;', '&lt;'], ['>', '<'], $_POST['kt_new']));
...
sub log_mente_act{
    ...
    while($line=<IN>){
        $line=~s/\n//g; $line .=$plus_fld;
        @Slog=preg_split(/<>/,$line,$Cfld+1);
        if($Slog[12] == "http://"){$Slog[12]="";}
        @pt=preg_split(/_/,$Slog[13]);
        foreach(0 .. 3){
            if(!$pt[$_]){$pt[$_]=0;}
        }
        $Slog[13]=join("_",@pt);
        pop(@Slog);
        @kt=preg_split(/&/,$Slog[10]);
        $kt=""; $del_fl=0; #削除フラグ
        foreach(@kt){
            if($ganes{$_}){$kt .=$_ . "&";}
        }
...
function cfg_set($EST_reg_fl = 0, $gane_st_fl = 0, $gane_guide_fl = 0, $exit_fl = 0)
{
    ...
    while ($tmp = fgets($fp, 4096)) {
        ...
        if ("function search_form(){\n" == $tmp) {
            $fl_sf = 1;
        } elseif ("} #end of &search_form\n" == $tmp) {
            $fl_sf = 0;
        } elseif ($fl_sf) {
            $PR_search_form .= $tmp;
        } elseif ("function menu_bar(){\n" == $tmp) {
            $fl_mb = 1;
        } elseif ("} #end of &menu_bar\n" == $tmp) {
            $fl_mb = 0;
        } elseif ($fl_mb) {
            $PR_menu_bar .= $tmp;
        } elseif ("function head_sp(){\n" == $tmp) {
            $fl_hs = 1;
        } elseif ("} #end of &head_sp\n" == $tmp) {
            $fl_hs = 0;
        } elseif ($fl_hs) {
            $PR_head_sp .= $tmp;
        } elseif ("function foot_sp(){\n" == $tmp) {
            $fl_fs = 1;
        } elseif ("} #end of &foot_sp\n" == $tmp) {
            $fl_fs = 0;
        } elseif ($fl_fs) {
            $PR_foot_sp .= $tmp;
        }
...
function mes($MES, $TITLE = '', $arg3 = '')
{
    ...
    if ('java' == $arg3) {
        ...
        $BACK_URL = "【<a href=\"{\Xmf\Request::getString('HTTP_REFERER', '', 'SERVER')}\">戻る</a>】";
    } elseif ('kanri' == $arg3) {

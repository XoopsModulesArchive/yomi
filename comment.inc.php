...
if (file_exists(XOOPS_ROOT_PATH . '/include/old_functions.php')) {
    require_once XOOPS_ROOT_PATH . '/include/old_functions.php';
}
//--------------//
$lid = $_GET['item_id'];
...
if (!isset($mode) || '' == $mode || ('nocomments' != $mode && 'thread' != $mode && 'flat' != $mode)) {
    if ($xoopsUser) {
...
if ('nocomments' != $mode && 'thread' != $mode && 'flat' != $mode) {
    $mode = 'flat';
}
...
$orderby = (1 == $order) ? 'date DESC' : 'date ASC';
if ('flat' == $mode) {
    $criteria = ['item_id=' . $lid . ''];

    $commentsArray = $yomicomment->getAllComments($criteria, true, $orderby);
} elseif ('thread' == $mode) {
    $criteria = ['item_id=' . $lid . '', 'pid=' . $yomicomment->getVar('pid') . ''];

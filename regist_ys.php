...
$Eref = Request::getString('HTTP_REFERER', '', 'SERVER');
Request::getString('HTTP_REFERER', '', 'SERVER') = '';
#管理画面のGETqueryをPOSTに変換。
if ('admin' == $_GET['changer'] && 'mente' == $_GET['in_mode'] && 'mente' == $_GET['Smode_name']) {
...
if (!isset($_POST['in_mode'])) { #新規登録()
    $Pdata = ['', '', 'http://', '', '', '', '', '', '', '', $_POST['kt'], '', 'http://', '', '', ''];
} elseif ('new_dairi' == $_POST['in_mode']) { #管理人代理登録(new_dairi)
} elseif ('mente' == $_POST['in_mode']) { #内容変更(mente)
    $query = "SELECT * FROM $EST[sqltb]log WHERE id='$_POST[id]' LIMIT 1";

    $result = $xoopsDB->query($query) || die('Query failed');

    $Pdata = $GLOBALS['xoopsDB']->fetchRow($result);

    if ($Pdata) {
        if (1 != $is_admin) {
            $cr_pass = crypt($_POST['pass'], $Pdata[5]);

            if (('admin' != $_POST['changer'] && $Pdata[5] != $cr_pass) || (!$_POST['pass'])) {
                mes('パスワードが違います', 'パスワード認証エラー', 'java');
            }
        }
    } else {
        mes('該当するデータはありません', 'エラー', 'java');
    }
} elseif ('form' == $_POST['in_mode']) { #外部入力(form)
    $_POST['Fkt'] = '';
    ...
    $Pdata = ['', $_POST['Ftitle'], $_POST['Furl'], '', '', $_POST['Fpass'], $_POST['Fsyoukai'], '', $_POST['Fname'], $_POST['Femail'], $_POST['Fkt'], '', $_POST['Fbana_url'], '', '', $_POST['Fkey'], '', $_POST['Fuid']];
} else {
    $Pdata = [];
}
...
#(1)プレビュー画面(preview)
if (isset($_POST['preview']) && 'on' == $_POST['preview']) {
    #※登録者の新規登録時にのみ使用
...
EOM;
    ...
    if ($_POST['id'] && (mb_strstr($_SERVER['HTTP_USER_AGENT'], 'Mozilla') || mb_strstr($_SERVER['HTTP_USER_AGENT'], 'Lynx') || mb_strstr($_SERVER[HTTP_USER_AGENT], 'Opera'))) {
        $ip_fl = 1;
        ...
        if ($EST['no_link_ip']) {
            ...
            foreach ($ip as $tmp) {
                if (mb_strstr($_SERVER['REMOTE_ADDR'], $tmp)) {
                    $ip_fl = 0;
        ...
        if ($ip_fl) {
            ...
            if (!$fl) {
                mes('「通知種別」に最低一つはチェックしてください', 'チェックミス', 'java');
            }
            ...
            if (mb_strlen("$Dcom$Dname$Demail") > 500) {
                mes('コメント、お名前、E-Mailの文字数は<br>合計で250文字(全角換算)までで、お願いします。', '文字数オーバー', 'java');
            }

            $fp = fopen("$EST[log_path]no_link_temp.cgi", 'ab');

            flock($fp, LOCK_EX);

            fwrite($fp, "$_POST[id]<>$_SERVER[REMOTE_ADDR]<>$Dhoukoku<>$Dcom<>$Dname<>$Demail<>\n");
    ...
    mes('ご報告ありがとうございました<br>管理人に「<b>' . htmlspecialchars($_POST['title'], ENT_QUOTES | ENT_HTML5) . '</b>」についての通知を行いました', 'ご報告ありがとうございます', $_POST['ref']);
...
elseif ('act_regist' == $_POST['mode']) {
    ...
    if ('admin' == $_POST['changer'] && 1 != $is_admin) {
    ...
    if ($EST_reg['nijyu_url']) {
        $new_id = get_id_url_ch(1);
    } else {
        ...
        $fp = fopen("$EST[log_path]$EST[temp_logfile]", 'rb');
    ...
    if ($EST['user_check'] && 'admin' != $_POST['changer'] && 'act_regist' == $_POST['mode']) { #<仮登録時>
        #仮登録ログデータに追加書き込み
        ...
        $fp = fopen("$EST[log_path]$EST[temp_logfile]", 'ab');
        ...
        $result = $xoopsDB->query($query) || die('Query failed regist633');
        ...
        if (($_POST['Fadd_kt'] || $_POST['Fto_admin']) && $EST_reg['look_mes'] && preg_match("/(\d+)(\w*)/", $EST_reg['look_mes'], $match)) {
            ...
            $fp = fopen("$EST[log_path]look_mes.cgi", 'rb');
...
EOM;
            ...
            $look_mes = implode('<>', $look_mes);

            $look_mes = str_replace("\n", '', $look_mes);

            $look_mes .= "<>\n";

            array_unshift($look_mes_list, $look_mes);

            $fp = fopen("$EST[log_path]look_mes.cgi", 'wb');
        ...
        ##メールを送信
        if ('no' != $_POST[FCmail] or 'admin' != $_POST['changer']) { #送信する設定なら
            #件名に付けるマークを設定
...
elseif ('enter' == $_GET['mode']) {
    ...
    if (($CK_data[4] && $CK_data[3] || 1 == $is_admin || (($x_uid) && $Pdata[17] == $x_uid)) && $_POST['id']) { #直接認証
    ...
    #regist;
        ...
        if ($_POST['id']) {
            ...
            $Tlog = $GLOBALS['xoopsDB']->fetchRow($result);

            if ($Tlog[0] == (string)$_POST[id]) {
                $i = 1;
            }

            $Tlog = array_map('stripslashes', $Tlog);
...
elseif ('help' == $_GET['mode']) {
    require "$EST[temp_path]help.html";

    exit;
} #(6)登録内容変更(act_mente)
elseif ('act_mente' == $_POST['mode']) {
    #$new=>追加データ書き込み用/$TASK=>更新するカテゴリリスト#
    ...
    } elseif (!$is_admin && $EST_reg['no_mente']) {
    ...
    $Spre_log = $GLOBALS['xoopsDB']->fetchRow($result);

    #登録者のパスワード認証

    if ('admin' != $_POST[changer]) {
        #$cr_pass=crypt($_POST[pass],$Spre_log[5]);

        if ($Spre_log[5] != $_POST['Fpass']) {
            mes('パスワードが間違っています' . $_POST['Fpass'] . " $Spre_log[5]", 'パスワード認証エラー', 'java');
        }
    }
...
elseif ('act_repass' == $_POST['mode']) {
    if ('repass' == $_POST['repass_mode']) { #パスワード再発行時
        if ('on' != $_POST['repass_check']) {
    ...
    $Slog = $GLOBALS['xoopsDB']->fetchRow($result);

    if ($Slog) {
        if ('repass' != $_POST['repass_mode']) {
            if ('admin' != $_POST['changer']) {
        ...
        $result = $xoopsDB->queryF($query) || die('Query failed1');
        ...
        mes($PR_mes, 'パスワード変更完了', (string)$EST[home]);
    } else {
...
elseif ('act_del' == $_POST['mode']) {
    ...
    */
    ...
    if ('single' == $_POST[del_mode]) { #del_mode:single
        if ('on' != $_POST['del_check']) {
            mes('削除確認のためにチェックを入れてから削除ボタンを押してください', '確認チェックをしてください', 'java');
        }

        if ('admin' != $_POST['changer'] && $EST_reg['no_mente']) {
            mes('現在、登録者による修正・削除は停止されています', 'エラー', 'java');
        }
        ...
        if ($Slog) {
            if (!$x_uid || ('admin' != $_POST['changer'] && $x_uid != $_POST['Fuid'])) { #削除する人が登録者の場合
                $cr_pass = crypt($_POST['pass'], $Slog[0]);

                if ($cr_pass != $Slog[0] || (!$_POST['pass'])) {
            ...
            $result = $xoopsDB->queryF($query) || die('Query failed regist916');
        ...
        if ('on' == $_POST['no_link']) {
            ...
            $fp = fopen("$EST[log_path]no_link.cgi", 'rb');
            ...
            $fp = fopen("$EST[log_path]no_link.cgi", 'wb');
        ...
        if ($_POST[del]) {
            foreach ($_POST['del'] as $del) {
                ...
                $result = $xoopsDB->queryF($query) || die("Query failed regist558 $query");
            }
        }
    }

    if ('admin' == $_POST['changer'] && ('on' == $_POST['no_link'] || 'on' == $_POST['dl_check'])) {
        mes('削除処理が完了しました', '削除完了', 'kanri');
    } else {
        mes('削除処理が完了しました', '削除完了', $EST[home]);
    }
...
#(9)登録画面(新規登録・管理人代理登録・登録内容変更)(regist)
#クッキーを記録
if ('mente' == $_POST['in_mode']) { #登録内容変更時
    $CK_data = get_cookie();

    if ('admin' != $_POST['changer'] && $_POST['pass']) {
        $CK_data[0] = $_POST['pass'];
    } #登録者パスワード
...
#管理人のみが登録できるモード
if ((1 == $EST_reg['no_regist']) && 'mente' != $_POST['in_mode'] && 'admin' != $_POST['changer']) {
    mes('現在、訪問者による新規登録は停止されています', 'エラー', 'java');
}
##$Smode_nameの設定
#管理人代理登録
if ('admin' == $_POST['changer'] && 'mente' != $_POST['in_mode']) {
    $Smode_name = 'new_dairi';
} #登録内容変更
elseif ('mente' == $_POST['in_mode']) {
    $Smode_name = 'mente';
} #登録者の新規登録
else {
    $Smode_name = '';
...
##$Pmodeの設定
#登録内容変更
if ('mente' == $_POST['Smode_name']) {
    $Pmode = 'act_mente';
} #新規登録
else {
    if (1 == $is_admin && !$_POST['id']) {
        $Smode_name = 'new_dairi';
...
$MES_sougo[0] = '';
#テンプレートの読み込み
if ('new_dairi' == $Smode_name) {
    if (!$Pdata[10]) {
        $Pdata[10] = '&' . $_GET['kt'];
    }

    require "$EST[temp_path]regist_new_admin.html";
} elseif ('admin' != $_POST['changer'] && 'mente' == $Smode_name) {
    if ($EST['syoukai_br']) {
    ...
    if ($xoopsUser) {
        ...
        if (!$Pdata[9]) {
            $Pdata[9] = $xoopsUser->email();
        }

        if (!$Pdata[17]) {
            $Pdata[17] = $xoopsUser->uid();
        }
...
function PR_kt($category = '')
{
    ...
    for ($kt_no = 1; $kt_no <= $EST_reg['kt_max']; $kt_no++) {
        ...
        <select name=Fkt<?= $kt_no ?> size=1>
            <?php
            if ($Pkt[$kt_no]) {
                echo '<option value="' . $Pkt[$kt_no] . "\"$PRselect>" . full_kt($Pkt[$kt_no]) . "\n";
            ...
            <option value=""<?= $PRselect ?>>--指定しない--
                ...
                foreach ($ganes as $line => $val) {
                    if ('admin' == $_POST['changer'] || !$gane_UR[$line]) {
...
function mes($MES, $TITLE = '', $arg3 = '')
{
    ...
    if ('java' == $arg3 || ('back_reg' == $arg3 && 'act_mente' == $_POST['mode'])) {
        ...
        $BACK_URL = '';
    } elseif ('back_reg' == $arg3) {
        $_POST['Fsyoukai'] = str_replace('<br>', "\n", $_POST['Fsyoukai']);

        if ('admin' == $_POST['changer']) {
            $_POST[in_mode] = 'new_dairi';
        } else {
            $_POST['in_mode'] = 'form';
        }
...
function check()
{
    ...
    if ($EST_reg['Fname'] && !$_POST['Fname']) {
        mes('<b>お名前</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
    }

    $num = mb_strlen($_POST['Fname']) - $EST_reg['Mname'] * 2;

    if ($num > 0) {
        mes("<b>お名前</b>は全角<b>$EST_reg[Mname]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
    }
    ...
    if ($EST_reg['Femail'] && !$_POST['Femail']) {
        mes('<b>メールアドレス</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
    } elseif (mb_strlen($_POST['Femail']) - $EST_reg['Memail'] > 0) {
        $num = mb_strlen($_POST['Femail']) - $EST_reg['Memail'];

        mes("<b>メールアドレス</b>は半角<b>$EST_reg[Memail]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
    } elseif ($EST_reg['Femail'] && !preg_match("/(.+)\@(.+)\.(.+)/", $_POST['Femail'])) {
    ...
    if (!$xoopsUser) {
        if ('act_mente' != $_POST['mode']) {
            if (!$_POST['Fpass']) {
                mes('<b>パスワード</b>は<font color=red>!記入必須項目</font>です', '記入ミス', 'back_reg');
            } elseif ($num = mb_strlen($_POST['Fpass']) > 8) {
                $num = mb_strlen($_POST['Fpass']) - 8;

                mes('<b>パスワード</b>は半角<b>8</b>文字以内でご記入ください', "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
            } elseif ($_POST['Fpass'] != $_POST['Fpass2']) {
                mes('２回の<b>パスワード</b>入力が一致しませんでした', '入力ミス', 'back_reg');
            }
    ...
    if ($EST_reg['Furl'] && !$_POST['Furl']) {
        mes('<b>ホームページアドレス</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
    } elseif (mb_strlen($_POST['Furl']) - $EST_reg['Murl'] > 0) {
        $num = mb_strlen($_POST['Furl']) - $EST_reg['Murl'];

        mes("<b>ホームページアドレス</b>は半角<b>$EST_reg[Murl]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
    ...
    if ($EST_reg['bana_url']) {
        if ('http://' == $_POST['Fbana_url']) {
            $_POST['Fbana_url'] = '';
        }

        if ($EST_reg['Fbana_url'] && !$_POST['Fbana_url']) {
            mes('<b>タイトルバナーのURL</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
        } elseif (mb_strlen($_POST['Fbana_url']) - $EST_reg['Mbana_url'] > 0) {
            $num = mb_strlen($_POST['Fbana_url']) - $EST_reg['Mbana_url'];

            mes("<b>タイトルバナーのURL</b>は半角<b>$EST_reg[Mbana_url]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
    ...
    if ($EST_reg['Ftitle'] && !$_POST['Ftitle']) {
        mes('<b>ホームページのタイトル</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
    }

    if (mb_strlen($_POST['Ftitle']) - ($EST_reg['Mtitle'] * 2) > 0) {
        $num = mb_strlen($_POST['Ftitle']) - ($EST_reg['Mtitle'] * 2);

        mes("<b>ホームページのタイトル</b>は全角<b>$EST_reg[Mtitle]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
    }
    ...
    if ($EST_reg['Fsyoukai'] && !$_POST['Fsyoukai']) {
        mes('<b>ホームページの紹介文</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
    }

    if (mb_strlen($_POST['Fsyoukai']) - ($EST_reg['Msyoukai'] * 2) > 0) {
        $num = mb_strlen($_POST['Fsyoukai']) - ($EST_reg['Msyoukai'] * 2);

        mes("<b>ホームページの紹介文</b>は全角<b>$EST_reg[Msyoukai]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
    }
    ...
    {#local(%kt_fl,$i,$j=0,$PR_kt);
        ...
        for ($i = 1; $i <= $EST_reg['kt_max']; $i++) {
            ...
            if ('admin' != $_POST[changer] && $gane_UR[$_POST["Fkt$i"]]) {
                mes('登録者の登録ができないカテゴリに変更しようとしています', 'カテゴリ選択ミス', 'back_reg');
            }
        }
        $j = count($kt_fl);
        if ($EST_reg['kt_min'] == $EST_reg['kt_max']) {
            $PR_kt = "<b>$EST_reg[kt_max]</b>個";
        } else {
            $PR_kt = "<b>$EST_reg[kt_min]</b>～<b>$EST_reg[kt_max]</b>個";
        }
        if ($EST_reg['kt_min'] > $j || $j > $EST_reg['kt_max']) {
    ...
    if ($EST_reg['Fkey'] && !$_POST['Fkey']) {
        mes('<b>キーワード</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
    }

    if (mb_strlen($_POST['Fkey']) - ($EST_reg['Mkey'] * 2) > 0) {
        $num = mb_strlen($_POST['Fkey']) - ($EST_reg['Mkey'] * 2);

        mes("<b>キーワード</b>は全角<b>$EST_reg[Mkey]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
    }
    ...
    if ('act_mente' != $_POST['mode'] && 'admin' != $_POST['changer']) {
        if ($EST_reg['Fadd_kt'] && !$_POST['Fadd_kt']) {
            mes('<b>追加して欲しいカテゴリ</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
        }

        if (mb_strlen($_POST['Fadd_kt']) - ($EST_reg['Madd_kt'] * 2) > 0) {
            $num = mb_strlen($_POST['Fadd_kt']) - ($EST_reg['Madd_kt'] * 2);

            mes("<b>追加して欲しいカテゴリ</b>は全角<b>$EST_reg[Madd_kt]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
        }
    ...
    if ('act_mente' != $_POST['mode'] && 'admin' != $_POST['changer']) {
        if ($EST_reg['Fto_admin'] && !$_POST['Fto_admin']) {
            mes('<b>管理人へのメッセージ</b>は<font color=red>記入必須項目</font>です', '記入ミス', 'back_reg');
        }

        if (mb_strlen($_POST['Fto_admin']) - ($EST_reg['Mto_admin'] * 2) > 0) {
            $num = mb_strlen($_POST['Fto_admin']) - ($EST_reg['Mto_admin'] * 2);

            mes("<b>管理人へのメッセージ</b>は全角<b>$EST_reg[Mto_admin]</b>文字以内でご記入ください", "文字数オーバー(半角換算で${num}文字分)", 'back_reg');
        }
...
function join_fld($arg = '')
{
    ...
    ##マークデータ(3)
    if ('admin' == $_POST['changer']) { #変更者が管理人
        $_POST['Fmark'] = '';
        ...
        $_POST['Fmark'] = mb_substr($_POST['Fmark'], 0, -1);
    ...
    ##カテゴリ(10)
    if ($EST[user_change_kt] && 'act_mente' == $_POST['mode'] && 'admin' != $_POST['changer']) { #登録者の変更でカテゴリ変更禁止の場合
        $i = 0;
    ...
    if ('on' == $_POST['Fhold_timestamp']) {
        ...
        if ('mente' == $Smode_name) { #内容変更時
            //	if(!$Spre_log[13] && $times-$Spre_log[11]<$EST[new_time]*86400){
    ...
    if ($EST['user_check'] && 'admin' != $_POST['changer'] && 'act_regist' == $_POST['mode']) {
        $Slog[14] = implode('<1>', [$_POST['Fsougo'], $_POST['Fadd_kt'], $_POST['Fto_admin']]);
    }
...
function get_id_url_ch($fl)
{
    ...
    if (!empty($_POST['Furl'])) {
        ...
        if ($Tlog) {
            ...
            if ($fl <= $i) {
                mes("そのURLはすでに登録されています<br><br>$Tlog[1] :<br>$Tlog[2]", '２重登録エラー', 'java');
            }

            if ('2' == $fl && '1' == $i && $Spre_log[2] != $_POST['Furl']) {
                mes("そのURLはすでに登録されています<br><br>$pre_title :<br>$_POST[Furl]", '２重登録エラー', 'java');
            }
        }
    }
    ...
    while ($line = fgets($fp, 4096)) {
        ...
        if ($EST['user_check'] && 'act_regist' == $_POST['mode']) {
            #仮登録モードでユーザの新規登録時
